...
/**
 * BoxBraidsRenderer component renders instanced 3D hair segments for Box Braids
...
export function BoxBraidsRenderer({ stylePos, lengthPos, thicknessPos, hairPlacementPoints }) {
    ...
    const { STYLE_COLORS, THICKNESS_MAP, showBraids, showOnlyRoots } = useHairStore(useShallow(state => ({
    ...
    useEffect(() => {
        ...
        hairPlacementPoints.forEach((point) => {
            ...
            const displaySegments = showOnlyRoots ? segments.slice(0, 1) : segments;
            const N_segs = displaySegments.length;
            const drawPos = new THREE.Vector3();
            if (N_segs > 0) {
                drawPos.copy(displaySegments[0].pos);
            }

            displaySegments.forEach((seg, k) => {
                let taper = 1.0;

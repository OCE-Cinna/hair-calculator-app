...
### Agentic Skills Application
- **Humanizer (`skills/humanizer.md`)**: ALWAYS apply this skill to your conversational responses with the user. Drop robotic AI-isms ("I have performed a comprehensive...", "Furthermore..."). Be direct, have a pulse, vary your sentence lengths, and sound like a real developer pair-programming.
- **Frontend Design (`skills/frontend/frontend-design.md`)**: Automatically apply when modifying UI components. Push for bold typography (Outfit/Inter), OKLCH colors, and premium aesthetics.
- **ThreeJS Optimization (`skills/threejs/`)**: Automatically apply when working in `Experience.jsx` to ensure 30+ FPS via GPU-side logic and proper memory disposal.
- **Web Component Design (`skills/react/web-component-design.md`)**: Apply when refactoring React code into Compound Components.

### Agent Workflows
Automatically apply the following workflows based on the user's task:
- **Code Review (`@[/code-reviewing-agent]`)**: Apply when asked to review code for consistency, cleanliness, and adherence to best practices. Use this before finalizing major feature implementations.
- **Debugging (`@[/debugging-agent]`)**: Apply when a bug, runtime error, or unexpected behavior is reported. Use systematic abductive reasoning to find the root cause before writing fixes.
- **Refactoring (`@[/refactoring-agent]`)**: Apply when asked to clean up technical debt, improve loop efficiency, or restructure existing code (e.g., pruning state or optimizing renders).
- **Tailwind CSS (`@[/tailwind-agent]`)**: Apply when styling components or implementing design system refinements in `index.css` or Tailwind utility classes.
- **Testing (`@[/test-writing-agent]`)**: Apply when asked to write or update unit/integration tests for components or calculation logic.
